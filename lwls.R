...
lwls_2d = function(bandwidth, kernel,
                   x_in_1 = as.numeric(rownames(y_in)),
                   x_in_2 = as.numeric(colnames(y_in)),
                   y_in, count_in,
                   x_out_1 = x_in_1, x_out_2 = x_in_2, power = 1, cv_mode = c(0,0)) {
  ...
  middle_stage = lwls_seq(bandwidth[1], kernel, x_in_1, y_in, count_in,
                          weight_out_need = TRUE, x_out_1, power, cv_mode[1])
  ...
  ans = matrix(lwls_seq(bandwidth[2], kernel, x_in_2, newy, neww,
                        weight_out_need = FALSE, x_out_2, power, cv_mode[2])$output,
...
bandwidth_choice_2d = function(kernel, data, cv_mode, symmetric = TRUE) {
  ...
  lo = max(apply(data$newy, 2, function(x) {find_min_bandwidth(x_in[!is.na(x)])}))
...
bandwidth_choice_2d_test = function(kernel, y, count, power = 1, cv_mode = c(cv_mode, 0), symmetric = TRUE) {
  # non-symmetric version
  ad_hoc_lwls = function(bw) {
    lwls_2d(c(bw, NA), kernel, y_in = y, count_in = count, power = 1, cv_mode = c(cv_mode, 0))
  }
  x_in = as.numeric(rownames(y))
  bw1 = optimize(ad_hoc_lwls,
                 lower = find_min_bandwidth(x_in),
                 upper = find_max_bandwidth(x_in))$minimum
  # ad_hoc_lwls = function(bw) {
  #   lwls_2d_vanilla(c(bw1, bw), kernel, data, power = 1, cv_mode = c(0, cv_mode))
  # }
  # bw2 = optimize(ad_hoc_lwls, lower = find_min_bandwidth(x_in), upper = find_max_bandwidth(x_in))$minimum
